...
class HibernateItemReadJobTest {
    ...
    void 하이버네이트_커서_리더_테스트() {
        //when
        JobExecution execution = jobLauncher.launchJob(HibernateCursorItemReadJob.JOB_NAME,
    ...
    void 하이버네이트_페이징_리더_테스트() {
        //when
        JobExecution execution = jobLauncher.launchJob(HibernatePagingItemReadJob.JOB_NAME,
                new JobParametersBuilder()
                        .addString("city", "인천")
                        .toJobParameters()
        );
